...
--
-- Client :  127.0.0.1
-- Généré le :  Mar 24 Novembre 2015 à 19:13
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `INSERT_COPY_PLAN_CADRE`(IN `PCODECOURS` CHAR(10), IN `PETAT` VARCHAR(30), IN `PPRENSENTATIONCOURS` VARCHAR(100), IN `POBJECTIFSINTEGRATION` VARCHAR(100), IN `PEVALUATIONAPPRENTISSAGE` VARCHAR(100), IN `PENONCECOMPETENCES` VARCHAR(100), IN `POBJECTIFSAPPRENTISSAGE` VARCHAR(100), IN `PMANUELOBLIGATOIRE` VARCHAR(100), IN `PRECOMMANDATION` VARCHAR(100))
    NO SQL
BEGIN
	INSERT INTO plancadre (Code_CodeCours, Etat, DateAjout, Presentation_Cours, Objectifs_Integration, Evaluation_Apprentissage, Enonce_Competences, Objectifs_Apprentissage, Manuel_Obligatoire, Recommandation)
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SELECT_PLAN_CADRE_INFOS`(IN `PCODECOURS` CHAR(10))
    NO SQL
BEGIN
    ...
    WHERE Courd_CodeCours = PCODECOURS;
END$$
...
(4, '340-101-MQ', 'elaboration', NULL, '2015-11-18', '../plancadre/../plancadre/4_340-101-MQ_presentation.txt', '../plancadre/../plancadre/4_340-101-MQ_integration.txt', '../plancadre/../plancadre/4_340-101-MQ_evaluation.txt', '../plancadre/../plancadre/4_340-101-MQ_competences.txt', '../plancadre/../plancadre/4_340-101-MQ_apprentissage.txt', NULL, NULL);
