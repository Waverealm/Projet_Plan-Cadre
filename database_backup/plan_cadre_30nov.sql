...
--
-- Client :  127.0.0.1
-- Généré le :  Lun 30 Novembre 2015 à 08:39
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SELECT_ALL_PLAN_CADRE`()
    NO SQL
BEGIN
  SELECT VersionPlan, CodeCours, NomCours, CodeProgramme, NomProgramme, Etat, plancadre.DateAjout, DateAdoption, Presentation_Cours, Objectifs_Integration, Evaluation_Apprentissage Enonce_Competences, Objectifs_Apprentissage, Manuel_Obligatoire, Recommandation FROM plancadre
  INNER JOIN cours ON Cours_CodeCours = CodeCours
  INNER JOIN programme on Programme_CodeProgramme = CodeProgramme
  ORDER BY CodeCours;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `SELECT_ALL_PLAN_CADRE_OFFICIEL`(IN `PETAT` VARCHAR(30))
    NO SQL
BEGIN
  SELECT VersionPlan, CodeCours, NomCours, CodeProgramme, NomProgramme, Etat, plancadre.DateAjout, DateAdoption, Presentation_Cours, Objectifs_Integration, Evaluation_Apprentissage Enonce_Competences, Objectifs_Apprentissage, Manuel_Obligatoire, Recommandation FROM plancadre
  INNER JOIN cours ON Cours_CodeCours = CodeCours
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SELECT_PLAN_CADRE_INFOS`(IN `PVERSIONPLAN` INT(11))
    NO SQL
BEGIN
    ...
    WHERE VersionPlan = PVERSIONPLAN;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UPDATE_STATE_PLANCADRE`(IN `PVERSIONPLAN` INT(11), IN `PETAT` VARCHAR(30))
    NO SQL
BEGIN
	UPDATE plancadre
    SET Etat = PETAT
...
('340-101-MQ', 'Philosophie et rationalité', NULL, '3-1-3', '1.000000', 20, '111.GE', '2015-10-04'),
('393-DE0-LG', 'Catalogage', 'cours du programme', '2-2-1', '1.333330', 30, '393.A0', NULL),
('420-EDA-05', 'Notions de base en informatiqu', 'cours du programme', '2-3-1', '0.666670', 30, '393.A0', '2015-10-05');
...
CREATE TABLE IF NOT EXISTS `plancadre` (
  ...
  `EtatOfficiel` varchar(50) NOT NULL,
...
INSERT INTO `plancadre` (`VersionPlan`, `Cours_CodeCours`, `Etat`, `EtatOfficiel`, `DateAdoption`, `DateAjout`, `Presentation_Cours`, `Objectifs_Integration`, `Evaluation_Apprentissage`, `Enonce_Competences`, `Objectifs_Apprentissage`, `Manuel_Obligatoire`, `Recommandation`) VALUES
(4, '340-101-MQ', 'Élaboration', '', NULL, '2015-11-18', '../plancadre/../plancadre/4_340-101-MQ_presentation.txt', '../plancadre/../plancadre/4_340-101-MQ_integration.txt', '../plancadre/../plancadre/4_340-101-MQ_evaluation.txt', '../plancadre/../plancadre/4_340-101-MQ_competences.txt', '../plancadre/../plancadre/4_340-101-MQ_apprentissage.txt', NULL, NULL),
(78, '420-EDA-05', 'Élaboration', '', NULL, '2015-11-24', '../plancadre/../plancadre/3_420-EDA-05_presentation.txt', '../plancadre/../plancadre/3_420-EDA-05_integration.txt', '../plancadre/../plancadre/3_420-EDA-05_evaluation.txt', '../plancadre/../plancadre/3_420-EDA-05_competences.txt', '../plancadre/../plancadre/3_420-EDA-05_apprentissage.txt', NULL, NULL),
(89, '393-DE0-LG', 'Élaboration', '', NULL, '2015-11-24', NULL, NULL, NULL, NULL, NULL, NULL, NULL);
