...
--
-- Client :  127.0.0.1
-- Généré le :  Mer 11 Novembre 2015 à 16:06
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `INSERT_USER`(IN `PUSERNAME` VARCHAR(20), IN `PMOTDEPASSE` VARCHAR(225), IN `PEMAIL` VARCHAR(50), IN `PNOM` VARCHAR(30), IN `PPRENOM` VARCHAR(30), IN `PTYPEUTILISATEUR` VARCHAR(30), IN `PETAT` VARCHAR(20))
    NO SQL
BEGIN
...
DROP PROCEDURE IF EXISTS `SELECT_PASSWORD`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SELECT_PASSWORD`(IN `PUSERNAME` VARCHAR(20))
    NO SQL
BEGIN
	SELECT MotDePasse FROM utilisateur WHERE Username = PUSERNAME;
END$$
...
SET EnonceConsigne = PEnonceConsigne,
DescriptionConsigne = PDescriptionConsigne
...
CREATE TABLE IF NOT EXISTS `utilisateurs` (
  ...
  `MotDePasse` varchar(225) DEFAULT NULL,
